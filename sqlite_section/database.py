...
CREATE TABLE IF NOT EXISTS relations_v2 (
    ...
    FOREIGN KEY (from_entry_id) REFERENCES entries_v2(id) ON DELETE CASCADE,
    FOREIGN KEY (to_entry_id) REFERENCES entries_v2(id) ON DELETE CASCADE,
...
def delete_entry_v2(language, word):
    ...
    if cur.rowcount == 0:
        print("No entry ", word, " is found")
    else:
        print("Entry deleted:", word)
        print("Related relations were deleted automatically")
        # conn.rollback()
        # conn.close()
        # return
