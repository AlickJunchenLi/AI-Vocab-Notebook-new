...
The pointer acts like a small lamp held just above the page, and nothing is drawn at the cursor itself. As it nears the edge of a glass surface, the rim there swells and its highlight brightens, with a thin darker line along the edge so the light reads on pale glass; in a corner, both edges respond. The swell follows with a slight springy lag, and the light takes the current theme's hue. The **Glass** button in the header switches the cursor effect on or off and remembers your choice on this device. On smaller screens, it appears as a sparkle icon.
